"""
튜닝 버전: Prophet 학습/예측 (로그 변환 + 하이퍼파라미터 최적화)

src_optimized 대비 변경점:
  - hyperparameter_search.py 결과(hp_search_results.csv)를 자동 로드
  - CSV 없으면 그리드 서치 탐색 범위 내 기본값 사용 (cps=0.1, sps=10.0)
  - changepoint_prior_scale / seasonality_prior_scale 모두 튜닝 적용

실행: python src_tuned/train_predict.py
출력: models/request-rate-forecast-tuned.pkl
      models/runs.json  (기록 추가)
      data/predictions_tuned_request_rate.csv
...
MAPE_THRESHOLD         = 0.15
MODEL_NAME             = "request-rate-forecast-tuned"
...
def load_best_params() -> tuple[float, float]:
    ...
    if path.exists():
        ...
        print(f"[파라미터] hp_search_results.csv 로드  cps={cps}  sps={sps}")
    else:
        cps, sps = DEFAULT_CPS, DEFAULT_SPS
        print(f"[파라미터] 탐색 결과 없음 — 기본값 사용  cps={cps}  sps={sps}")
        print("           (먼저 python src_tuned/hyperparameter_search.py 실행 권장)")
    return cps, sps
...
def train(df: pd.DataFrame, cps: float, sps: float) -> tuple[Prophet, float]:
    df_log = log_transform(df)
    print(f"\n[학습] {MODEL_NAME}  (cps={cps}, sps={sps})")
...
def predict_annual(model: Prophet, year: int = 2025) -> pd.DataFrame:
    ...
    for offset, idx_val in [(-1, 0.1), (0, 0.9), (1, 0.7), (2, 0.3), (3, 0.1)]:
        ...
        future.loc[mask, "typhoon_index"] = np.maximum(future.loc[mask, "typhoon_index"].values, idx_val)

    raw      = model.predict(future)
    forecast = inverse_transform(raw[["ds", "yhat", "yhat_lower", "yhat_upper"]])
    forecast["required_pods"] = forecast["yhat"].apply(_to_pods)

    out = DATA_DIR / f"predictions_tuned_annual_{year}.csv"
...
def save_model(model: Prophet, mape: float, cps: float, sps: float) -> None:
...
def main():
    print("=" * 55)
    print("  튜닝 모델: Prophet + log 변환 + 하이퍼파라미터 최적화")
    print("=" * 55)

    cps, sps = load_best_params()
    df       = load_data()

    model, mape = train(df, cps, sps)
    save_model(model, mape, cps, sps)

    forecast_24h = predict_24h(model, df)
    out_24h = DATA_DIR / "predictions_tuned_request_rate.csv"
